  ...
  groupes: [
    {
      ...
      points: [
        ...
        { id:"temps-accelere", ou:"reglage-temps", regle:"vitesses",
        ...
        { id:"taille-sondes", ou:"reglages", regle:"taille-sondes",
        ...
        { id:"rythme-voyage", ou:"reglages", regle:"rythme-choix",
        ...
        { id:"vitesse-repere", ou:"recul",
          aveu:"Speed is counted relative to the solar system. Departure and arrival are both assumed at rest in it.",
          sources:["gravity2021"],
          t:"<b>Fast compared to what.</b> A speed means nothing on its own: it is always counted relative to something. The one on the flight strip is counted relative to the solar system — that is, relative to where you are going. The one-g flight is integrated in the frame where departure and arrival are both at rest, and that is where the simplification lies: Sagittarius A* and the Sun are not motionless with respect to each other — the Sun orbits the centre of the Galaxy. That speed is tiny next to the journey's, but it is not zero, and this site does not compute it. We ignore it, and we say so." },
        ...
        { id:"ciel-arrivee", ou:"arrivee-soleil",
          aveu:"These stars are the ones from back there. The sky of here is not computed yet.",
          sources:["gravity2021"],
          t:"<b>The sky at arrival.</b> This compromise changed its nature on 16 August 2026, and the change is worth telling. It used to say: the ship never travels, so the image still paints the black hole out in the bay, and we cover that view with a flat black in order to show the Earth. That veil has FALLEN. The ship really travels now that the journey runs in one piece: you arrive some twenty billion Schwarzschild radii out, where the black hole's shadow spans less than a fifty-billionth of a radian. There was nothing left to hide. What remains false is more honest said than veiled: the star field you find behind the Earth is the nuclear cluster's, computed for the sky of Sagittarius A*, not for ours. Those are not our constellations. The veil, for its part, cost more than it earned: a flat black stuck to the pane follows the window as you walk, and with no star to arbitrate, the eye concludes that the sky is following you — the exact opposite of what we meant to show." },
        ...
        { id:"face-astres", ou:"arrivee-soleil",
          aveu:"We do not know which face of the Earth is turned towards us. We show the origin of the coordinate frame.",
          sources:["jplPlanetes"],
          t:"<b>The face we are looking at.</b> The surfaces are real photographs — a MODIS composite for the Earth, a Lunar Reconnaissance Orbiter mosaic for the Moon — no longer hand-drawn blotches. But knowing WHICH face is turned towards us would take a date and an hour, which this site does not have: it is the same admission as the orbits, we know where a planet circles, not where it has got to. So the sub-observer point sits at the origin of the frame, zero longitude and zero latitude, because that is the only choice that is not a choice — picking the most flattering face would be choosing a picture over a fact. For the Moon this is not a compromise at all: its rotation is synchronous, it always shows us the same face, and that face is indeed zero longitude in its own frame." },
        { id:"rayon-terre", ou:"arrivee-soleil",
          aveu:"We descend along a radius that meets the Earth. We do not know where it really is on its orbit.",
          sources:["jplPlanetes"],
          t:"<b>The radius we descend.</b> The journey no longer stops: you cross the Oort cloud, fall towards the giants, carry on, and the Earth appears. For the fall and the arrival to describe the same point, we assume you descend along a radius that meets the Earth — so you arrive from outside with the Earth between you and the Sun, which puts it exactly where the bay is already looking. That is an assumption, not a measurement: we know no more about where the Earth is on its orbit than about Jupiter on its own. What IS computed is the moment of handover: the fall stops at the distance where the Earth's disc reaches half a pixel, the threshold below which this site never draws anything." },
        { id:"anneaux-solaires", ou:"arrivee-soleil",
          sources:["jplElementsPlanetes", "iau2015b3"],
          aveu:"These circles bound the gap to the Sun; they do not say where the planet is.",
          t:"<b>The rings of the solar system.</b> The columns taken from the JPL give each planet's semi-major axis and nothing else: neither where it sits on its orbit today, nor how that orbit is tilted as seen from the centre of the Galaxy. Placing a dot and writing \"Jupiter\" on it would be a claim nothing here supports. What is drawn is therefore the circle that <b>bounds</b> its gap to the Sun — the planet is somewhere inside, and the circle claims no more than that. The Sun's brightness is painted on the magnitude scale: the flux received is multiplied by twenty-seven thousand between the Oort cloud and the arrival, and no screen holds that ratio. Its diameter, on the other hand, is never drawn at all — it stays below half a pixel everywhere in this scene." },
