...
- [ ] 3.12 **sourcer le système solaire** — **BLOQUÉ PAR LE RÉSEAU dans la session du 10 août** : `ssd.jpl.nasa.gov`, `arxiv.org`, `adsabs.harvard.edu` et `aanda.org` sont tous refusés par le mandataire de sortie de l'environnement d'exécution. Seule la recherche fonctionne, et un résumé de moteur de recherche n'est pas une source relevée — la règle 7 dit de dériver ou de relever, pas de recopier. **Ce qui manque et qu'il faut aller chercher** : les demi-grands axes de Mercure à Neptune, la luminosité et la magnitude du Soleil, le nuage de Oort (bornes, et le fait qu'il n'a jamais été observé). Ce qui existe déjà, relevé avec DOI, dans le registre parallèle de `lune.js` : `jplPlanetes`, `jplSatellites`, `jplElements`, `iau2015b3` (rayon solaire nominal, (𝒢M)☉), `codata2018` — elles ne peuvent pas entrer dans `contenu.js` seules, le contrat refuse une source que personne ne cite. **Fait en attendant** : les constantes astronomiques sont gardées (`outil-verif-constantes.js`, 11 contrôles) — demi-grands axes, l'unité astronomique, rayon et luminosité du Soleil, sa magnitude, le nuage de Oort. Deux sources existent déjà **hors du contrat**, dans le registre parallèle de `lune.js:89-148`. Et **l'UA a cinq écrivains sans source** (`echelle.js:49`, `recul.js:45`, `voyage1g.js:49`, `lune.js:85`, `index.html:3877`) : c'est la maladie du disque à 622×. Plus une fiche « le système solaire vu du dehors », patron `f-ciel`
