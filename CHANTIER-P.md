...
- [ ] 3.10 **le système solaire vu de loin, avec ses étiquettes** — **CADRAGE TRANCHÉ PAR HUGO LE 10 AOÛT AU SOIR : on arrive dans le nuage de Oort, puis on TOMBE vers 1 000 ua.** Le chiffrage est dans `A-REGARDER.md` : depuis le nuage, huit planètes dans deux pixels — aucune étiquette n'a le droit d'exister, elles pointeraient le même point ; à 1 000 ua, Neptune est à 36 px du Soleil et Jupiter à 6, et il y a une scène. **Conséquence pour `etiquettes.js` : le mécanisme doit savoir SE TAIRE, pas seulement se placer** — et le passage d'un régime à l'autre est le sujet de la scène, pas un fondu de transition. C'est là que « rien à voir » devient « ah, il y a quelque chose ». — et c'est aussi ce qui débloque le rythme symétrique : compter les décades depuis Sagittarius, puis VERS le Soleil, demande qu'il y ait quelque chose à approcher — « une vision depuis le nuage dehors, avec des tags : ça c'est Jupiter, ça c'est truc — tout petit évidemment, on ne le verrait même pas, on verrait juste le Soleil ». L'honnêteté du site rend la chose belle : à cette distance il n'y a rien à voir, et les étiquettes disent où sont les choses qu'on ne voit pas.
